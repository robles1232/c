form.register(_dir_submodulo_local_platos,{
    nuevo: function(){
        get_modal(_dir_submodulo_local_platos, _prefix_local_platos)
    },
    editar: function(id){
        get_modal(_dir_submodulo_local_platos, _prefix_local_platos, "edit", id)
    },
    eliminar_restaurar: function(id, obj){
        ...
        swal({ title: "Confirmar", text: "¿Desea " + accion__ + " el registro seleccionado?", type: "warning", showCancelButton: !0, confirmButtonText: "Confirmar", cancelButtonText: "Cancelar" }, function() {
            ...
            $.ajax({
                url: route(_dir_submodulo_local_platos + '.destroy', 'delete'),
    ...
    guardar: function() {
        var $self = this
        let _form = "#form-" + _dir_submodulo_local_platos
        ...
        $.ajax({
            url: route(_dir_submodulo_local_platos + '.store'),
            type: 'POST',
            data: post_data,
            cache: false,
            contentType: false,
            ...
            success: function(response) {
                ...
                $self.callback(response)
                close_modal(_dir_submodulo_local_platos)
            },
            complete: function() {},
            error: function(e) {
                if (e.status == 422) { //Errores de Validacion
                    toastr.remove();
                    limpieza(_dir_submodulo_local_platos)
                    $.each(e.responseJSON.errors, function(i, item) {
                        if(i == "productos"){
                            toastr.warning(item, msj_modulo)
                        }

                        $('#'+ _prefix_local_platos+ "_" + i ).addClass('is_invalid')
                        $('#'+ _prefix_local_platos+ "_" + i ).attr('data-invalid', item)

                        $('.select2-' + _prefix_local_platos + "_" + i).addClass('select2-is_invalid');
                        $('.select2-' + _prefix_local_platos + "_" + i).attr('data-invalid', item);
    ...
    callback: function(data) {
        grilla.reload(_dir_submodulo_local_platos)
    }
})