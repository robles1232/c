        ...
        $.ajax({
            ...
            error: function(e) {
                if (e.status == 422) { //Errores de Validacion
                    ...
                    $.each(e.responseJSON.errors, function(i, item) {
                        $('#'+ _prefix_seguridad_funciones+ "_" + i ).addClass('is_invalid')
                        $('#'+ _prefix_seguridad_funciones+ "_" + i ).attr('data-invalid', item)

                        $('.select2-' + _prefix_seguridad_funciones + "_" + i).addClass('select2-is_invalid');
                        $('.select2-' + _prefix_seguridad_funciones + "_" + i).attr('data-invalid', item);
